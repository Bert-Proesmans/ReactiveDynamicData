...
namespace ReactiveDynamicData.Diff
{
    ...
    public class ListDiff
    {
        ...
        /// <summary>
        /// Compare two given lists and return their differences.
        /// </summary>
        /// <typeparam name="T">The type subject</typeparam>
        /// <param name="listLeft">Source list.</param>
        /// <param name="listRight">New list that will be compared against <paramref name="listLeft"/>.</param>
        /// <param name="timeSpan">The amount of time that is allowed to pass before the operation is stopped</param>
        /// <param name="diffDualThreshold">TODO</param>
        /// <returns>Ordered enumerable of objects containing difference introspections.</returns>
        public static List<Diff<T>> Compare<T>(
            ...
            int diffDualThreshold = 32)
        {
            ...
            return new ListDiff(timeSpan.Value, diffDualThreshold).CompareUsingOptions(listLeft, listRight, comparer);
        }

        internal List<Diff<T>> CompareUsingOptions<T>(IReadOnlyList<T> text1, IReadOnlyList<T> text2, IEqualityComparer<T> comparer)
        {
            // Check for equality using references (speedup)
            ...
            var commonlength = GetCommonPrefix(text1, text2, comparer);
            var commonprefix = text1.Substring(0, commonlength);
            text1 = text1.Substring(commonlength);
            text2 = text2.Substring(commonlength);

            // Trim off common suffix (speedup)
            commonlength = GetCommonSuffix(text1, text2, comparer);
            ...
            var diffs = Compute(text1, text2, comparer);
            ...
            CleanupMerge(diffs, comparer);
            return diffs;
        }

        private List<Diff<T>> Compute<T>(IReadOnlyList<T> text1, IReadOnlyList<T> text2, IEqualityComparer<T> comparer)
        {
            ...
            var i = longtext.IndexOf(shorttext, comparer);
            if (i != -1)
            {
            ...
            var hm = GetHalfMatch(text1, text2, comparer);
            if (hm != null)
            {
                ...
                var diffsA = CompareUsingOptions(text1A, text2A, comparer);
                var diffsB = CompareUsingOptions(text1B, text2B, comparer);
                // Merge the results.
                diffs = diffsA;
            ...
            diffs = GetMap(text1, text2, comparer);
        ...
        internal List<Diff<T>> GetMap<T>(IReadOnlyList<T> text1, IReadOnlyList<T> text2, IEqualityComparer<T> comparer)
        {
            ...
            for (var d = 0; d < maxD; d++)
            {
                ...
                for (var k = -d; k <= d; k += 2)
                {
                    ...
                    while (!done && x < text1Length && y < text2Length
                           && text1.EqualsAt(x, text2, y, comparer))
                    {
                        x++;
                        y++;
                        if (doubleEnd)
                ...
                if (doubleEnd)
                {
                    ...
                    for (var k = -d; k <= d; k += 2)
                    {
                        ...
                        while (!done && x < text1Length && y < text2Length
                               && text1.EqualsAt(text1Length - x - 1, text2, text2Length - y - 1, comparer))
                        {
                            x++;
                            y++;
                            footstep = GetFootprint(text1Length - x, text2Length - y);
        ...
        internal static int GetCommonPrefix<T>(IReadOnlyList<T> text1, IReadOnlyList<T> text2, IEqualityComparer<T> comparer)
        {
            // Performance analysis: http://neil.fraser.name/news/2007/10/09/
            var n = Math.Min(text1.Count, text2.Count);
            for (var i = 0; i < n; i++)
            {
                if (!text1.EqualsAt(i, text2, i, comparer))
                {
                    return i;
                }
            }
        ...
        internal static int GetCommonSuffix<T>(IReadOnlyList<T> text1, IReadOnlyList<T> text2, IEqualityComparer<T> comparer)
        {
            // Performance analysis: http://neil.fraser.name/news/2007/10/09/
            var text1Length = text1.Count;
            var text2Length = text2.Count;
            ...
            for (var i = 1; i <= n; i++)
            {
                if (!text1.EqualsAt(text1Length - i, text2, text2Length - i, comparer))
                {
                    return i - 1;
                }
            }
        ...
        internal static IReadOnlyList<T>[] GetHalfMatch<T>(IReadOnlyList<T> text1, IReadOnlyList<T> text2, IEqualityComparer<T> comparer)
        {
            var longtext = text1.Count > text2.Count ? text1 : text2;
            var shorttext = text1.Count > text2.Count ? text2 : text1;
            ...
            var hm1 = GetHalfMatchI(longtext, shorttext, (longtext.Count + 3) / 4, comparer);
            // Check again based on the third quarter.
            var hm2 = GetHalfMatchI(longtext, shorttext, (longtext.Count + 1) / 2, comparer);
            IReadOnlyList<T>[] hm;
        ...
        private static IReadOnlyList<T>[] GetHalfMatchI<T>(IReadOnlyList<T> longtext, IReadOnlyList<T> shorttext, int i, IEqualityComparer<T> comparer)
        {
            ...
            while (j < shorttext.Count && (j = shorttext.IndexOf(seed, comparer, j + 1)) != -1)
            {
                var prefixLength = GetCommonPrefix(longtext.Substring(i), shorttext.Substring(j), comparer);
                var suffixLength = GetCommonSuffix(longtext.Substring(0, i), shorttext.Substring(0, j), comparer);
        ...
        internal static void CleanupMerge<T>(List<Diff<T>> diffs, IEqualityComparer<T> comparer)
        {
            while (true)
            {
                        ...
                        case Operation.Equal:
                            ...
                            if (countDelete != 0 || countInsert != 0)
                            {
                                if (countDelete != 0 && countInsert != 0)
                                {
                                    ...
                                    var commonlength = GetCommonPrefix(textInsert, textDelete, comparer);
                                    if (commonlength != 0)
                                    {
                                    ...
                                    commonlength = GetCommonSuffix(textInsert, textDelete, comparer);
                                    if (commonlength != 0)
                                    {
                ...
                while (pointer < (diffs.Count - 1))
                {
                    if (diffs[pointer - 1].Operation == Operation.Equal && diffs[pointer + 1].Operation == Operation.Equal)
                    {
                        ...
                        if (diffs[pointer].Items.EndsWith(diffs[pointer - 1].Items, comparer))
                        {
                        ...
                        else if (diffs[pointer].Items.StartsWith(diffs[pointer + 1].Items, comparer))
                        {
